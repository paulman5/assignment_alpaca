...
- `place_buy_order(order_id, ticker, usdc_amount)` — creates a `PendingOrder` PDA at seeds `["order", user, order_id_le]` and emits `BuyOrderCreated`. Permissionless; sign with your own devnet keypair. `(user, order_id)` is the business key.
- `fulfill_buy_order(order_id, actual_usdc)` — closes the PDA, emits `BuyOrderFulfilled`. This is your worker's settlement call.
- `refund_buy_order(order_id)` — closes the PDA, emits `BuyOrderRefunded`. The terminal for orders that can't fill (e.g. market closed).
...
KEYPAIR=devnet-keypair.json npm run place-order -- AAPL 2500
```
